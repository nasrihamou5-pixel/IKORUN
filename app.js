...
// Le listener visibilitychange + ensurePush() (notifs prière/rappel) pouvaient
// tous deux déclencher subscribeToPush() dans la même seconde au chargement,
// doublant l'upsert push_subscriptions et loggant l'erreur deux fois en cas
// d'échec réseau. Un cooldown court absorbe les appels concurrents/rapprochés
// sans empêcher une vraie resynchronisation plus tard (ex: après setLang()).
let _pushSubInFlight=null, _pushSubLastAt=0;
async function subscribeToPush(){
  if(!('serviceWorker'in navigator) || !('PushManager'in window)) return false;
  if(!window.currentUserId || !window.supabaseClient) return false; // même un compte invité a un currentUserId
  if(_pushSubInFlight) return _pushSubInFlight;
  if(Date.now()-_pushSubLastAt<3000) return true;
  _pushSubInFlight=(async()=>{
    try{
  ...
  try{ return await _pushSubInFlight; }
  finally{ _pushSubLastAt=Date.now(); _pushSubInFlight=null; }
}
...
// reloadState() ET la suite finishLogin()->ensurePublicProfile().then(syncPublicProfile)
// appelaient toutes deux syncPublicProfile() au même démarrage : deux update
...
const VVVCrypto = (function(){
  ...
  async function getOrCreateKey(){
    ...
    await new Promise((res,rej)=>{
      const req=idb.transaction(STORE,'readwrite').objectStore(STORE).put(key,KEY_ID);
...
const DB = {
  ...
  save(k,v){
    // save() était appelée à chaque reloadState() pour re-normaliser l'état (profil,
...
function nav(s){
  ...
  // La barre du bas reste volontairement cliquable pendant le tour guidé (repère
  // constant, cf startAppTour()) — mais nav() ignorait totalement _tourOn : taper
  // un onglet pendant le tour laissait la carte/l'anneau affichés pour l'étape en
  // cours, maintenant ancrés sur un écran qui n'a plus rien à voir. On referme le
  // tour dès qu'une navigation manuelle a lieu, comme sur la plupart des apps.
  if(typeof _tourOn!=='undefined' && _tourOn && (typeof TOUR_STEPS==='undefined' || !TOUR_STEPS[_tourIdx] || TOUR_STEPS[_tourIdx].page!==s)) endTour();
...
function applyOnboardingLabels(){
  ...
  if(step5){
    ...
    // Empêche de choisir une date passée dans le sélecteur natif (le champ
    // n'avait aucun `min`, donc une date déjà passée générait un plan de
    // 2 semaines sans avertir personne — voir aussi la garde dans generatePlan()).
    const minD=new Date(); minD.setDate(minD.getDate()+7);
    $('#ob_compdate').min=minD.toISOString().slice(0,10);
  }
...
// Fajr/Dhuhr/Asr/Maghrib/Isha sont des clés internes (utilisées pour les calculs
// et comme index d'objet) affichées telles quelles jusqu'ici — jamais traduites,
// y compris en arabe où "Fajr" en alphabet latin sortait sur l'écran d'accueil.
const PRAYER_TR={
  fr:{Fajr:'Fajr',Dhuhr:'Dhouhr',Asr:'Asr',Maghrib:'Maghreb',Isha:'Icha'},
  en:{Fajr:'Fajr',Dhuhr:'Dhuhr',Asr:'Asr',Maghrib:'Maghrib',Isha:'Isha'},
  ar:{Fajr:'الفجر',Dhuhr:'الظهر',Asr:'العصر',Maghrib:'المغرب',Isha:'العشاء'}
};
function trPrayer(p){ const l=curLang(); return (PRAYER_TR[l]&&PRAYER_TR[l][p])||p; }
// "4h51"/"51 min" est un format latin qui ne se lit pas naturellement en arabe
// (unités "h"/"min" en alphabet latin dans une phrase RTL). En arabe on garde
// les chiffres (déjà utilisés partout ailleurs dans l'app arabe) mais avec les
// unités س (heure) / د (minute).
function fmtCountdown(h,m){
  if(curLang()==='ar') return h>0?(h+'س'+String(m).padStart(2,'0')):(m+' د');
...
function ACHIEVEMENTS_DEF(){ return [
  {key:'premiere',    name:t('ach_premiere_name'),  img:'badges/ach_premiere.png',    cat:'Accomplissement', cls:'bd-debutant', desc:t('ach_premiere_desc'),            auto:()=>SESS.some(s=>s.type==='Course')},
  {key:'cinqk',       name:t('ach_cinqk_name'),      img:'badges/ach_cinqk.png',       emoji:'run',            cat:'Accomplissement', cls:'bd-amateur',  desc:t('ach_cinqk_desc'),   auto:()=>SESS.some(s=>s.km>5)},
  {key:'dixk',        name:t('ach_dixk_name'),       img:'badges/ach_dixk.png',        emoji:'flag',            cat:'Accomplissement', cls:'bd-sportif',  desc:t('ach_dixk_desc'),  auto:()=>SESS.some(s=>s.km>10)},
  {key:'serie',       name:t('ach_serie_name'),      img:'badges/ach_serie.png',       cat:'Accomplissement', cls:'bd-athlete',  desc:t('ach_serie_desc'), auto:()=>bestStreak()>=30},
  {key:'denivele',    name:t('ach_denivele_name'),   img:'badges/ach_denivele.png',    cat:'Accomplissement', cls:'bd-expert',   desc:t('ach_denivele_desc'),      auto:()=>SESS.some(s=>(s.deniv||0)>200)},
  {key:'podium',      name:t('ach_podium_name'),     img:'badges/ach_podium.png',      cat:'Accomplissement', cls:'bd-elite',    desc:t('ach_podium_desc'), manual:true},
  {key:'objectif',    name:t('ach_objectif_name'),   img:'badges/ach_objectif.png',    cat:'Accomplissement', cls:'bd-maitre',   desc:t('ach_objectif_desc'), auto:achObjectifReached},
  {key:'nouveaupb',   name:t('ach_nouveaupb_name'),  img:'badges/ach_nouveaupb.png',   emoji:'medal',            cat:'Performance',      cls:'bd-legende', desc:t('ach_nouveaupb_desc'), auto:achNouveauPB},
  {key:'allure',      name:t('ach_allure_name'),     img:'badges/ach_allure.png',      cat:'Performance',      cls:'bd-sportif',  desc:t('ach_allure_desc'), auto:()=>SESS.some(s=>s.km>=3 && s.pace && s.pace!=='—' && parseTime(s.pace)>0 && parseTime(s.pace)<=600)},
  {key:'endurance',   name:t('ach_endurance_name'),  img:'badges/ach_endurance.png',   cat:'Performance',      cls:'bd-athlete',  desc:t('ach_endurance_desc'),  auto:()=>SESS.some(s=>s.km>=15)},
  {key:'puissance',   name:t('ach_puissance_name'),  img:'badges/ach_puissance.png',   cat:'Performance',      cls:'bd-expert',   desc:t('ach_puissance_desc'), auto:()=>achWeeklyCount(MSESS,3)},
  {key:'vo2max',      name:t('ach_vo2max_name'),     img:'badges/ach_vo2max.png',      emoji:'health',            cat:'Performance',      cls:'bd-elite',    desc:t('ach_vo2max_desc'), auto:()=>(getUserVDOT()||0)>50},
  {key:'force',       name:t('ach_force_name'),      img:'badges/ach_force.png',       cat:'Performance',      cls:'bd-maitre',   desc:t('ach_force_desc'), auto:()=>achWeeklySum(MSESS,'tonnage',20000)}
]; }
...
const BADGE_IMG_FILES={
  debutant:'badges/rank_debutant.png',
  amateur:'badges/rank_amateur.png',
  sportif:'badges/rank_sportif.png',
  athlete:'badges/rank_athlete.png',
  expert:'badges/rank_expert.png',
  elite:'badges/rank_elite.png',
  maitre:'badges/rank_maitre.png',
  legende:'badges/rank_legende.png'
};
function bdGlyph(key){
  const src=BADGE_IMG_FILES[key];
  if(!src) return '<span class="bd-emoji">'+ICN(badgeEmoji(key),28,'var(--e)')+'</span>';
  return '<img class="bd-glyph" src="'+src+'" alt="" draggable="false" loading="lazy" data-key="'+key+'" onerror="bdImgErr(this)">';
}
function badgeEmoji(key){ const b=BADGE_TIERS.find(x=>x.key===key); return (b&&ICONS[b.emoji])?b.emoji:'medal'; }
// Image embarquée en base64 : plus de retry réseau possible, on bascule direct
// sur l'icône si jamais le décodage échoue (data corrompue).
...
function legalTermsHTML(){
  ...
  +legalP('8 bis. Fonctionnalités payantes',
    'À ce jour, l’Application est intégralement gratuite et ne comporte aucun paiement ni aucun abonnement. Des fonctionnalités payantes sont envisagées à l’avenir ; si elles sont introduites, elles seront annoncées clairement avant toute souscription et les règles suivantes s’appliqueront. Le prix affiché sera en euros toutes taxes comprises. Un abonnement sera souscrit pour la période choisie et se renouvellera automatiquement à son terme, sauf résiliation avant la fin de la période en cours ; la résiliation prendra effet à l’échéance et l’accès restera ouvert jusque-là. Le paiement sera traité par un prestataire externe (par exemple Stripe) : l’Éditeur n’aura jamais accès à ton numéro de carte, seul le prestataire le traitera, conformément à sa propre politique de confidentialité. IKORUN reste distribuée exclusivement comme application web (PWA) et ne sera jamais publiée sur l’App Store ni sur le Play Store. Conformément à l’article L221-18 du code de la consommation, tu disposeras d’un délai de rétractation de quatorze jours à compter de la souscription ; toutefois, en demandant l’accès immédiat à une fonctionnalité payante, tu demanderas expressément l’exécution du contrat avant la fin de ce délai et renonceras à ton droit de rétractation pour la partie déjà exécutée, ce qui te sera rappelé au moment du paiement. En cas de défaut de paiement, l’accès aux fonctionnalités payantes serait suspendu, mais tes données et ton compte resteraient conservés et exportables. Aucune fonctionnalité déjà utilisée gratuitement ne deviendra payante de façon rétroactive sans préavis d’au moins trente jours.')
  +legalP('8 ter. Publicité',
    'À ce jour, l’Application n’affiche aucune publicité. Des espaces publicitaires sont envisagés à l’avenir pour financer le service sans faire reposer son fonctionnement uniquement sur les fonctionnalités payantes. S’ils sont introduits, ils seront toujours clairement identifiés comme tels (jamais déguisés en contenu ou en conseil sportif), sans jamais interrompre une séance en cours ni s’afficher pendant le suivi d’un entraînement actif. Une publicité pour un produit de santé, complément alimentaire ou dispositif médical serait exclue par principe, compte tenu de la nature sportive et santé de l’Application. Le recours à des publicités personnalisées (basées sur ton comportement) plutôt que contextuelles ferait l’objet d’une demande de consentement explicite et révocable à tout moment, conformément à la réglementation ePrivacy et au RGPD — voir l’article 3 bis de la Politique de confidentialité. Tu pourras toujours utiliser gratuitement les fonctionnalités existantes de l’Application même en refusant les publicités personnalisées.')
...
function pfAccountHTML(){
  ...
  return '<button class="btn" onclick="signInWithGoogle()">'+t('loginBtnLabel')+'</button>';
}
